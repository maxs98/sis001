...
class ThreadGetTids(threading.Thread):
	...
	def run(self):
		...
		attempts = 0
		while attempts < 3:
			topics_html = re.findall('<tbody.*?normalthread.*?>.*?</tbody>', html.content, re.M | re.S) #粗过滤，先过滤每个帖子的所有内容返回top_html只包含所有帖子的内容
			if topics_html == '':
...
def down_link_imgs_torrents(topic):
	...
	imgs = re.findall('\<img src\=\"(http\:\/\/.*?\.jpg|attachments\/.*?\.jpg)\"', html.content, re.M | re.S) #匹配这个页面所有的图片链接
	torrents = re.findall('a href=\"(?P<url>attach[=0-9a-zA-Z\.\?]+).*?>(?P<title>[^<>\"]*?torrent)', html.content, re.M | re.S) #匹配这个页面所有的种子链接
...
if __name__ == '__main__':
	...
	sis001 = sis001()
	t = dict()	
	fids = {"版面代码":25} #需要采集的版面{"典藏":411,"熟妇":242,"自拍":62,"若兰居":495,"亚洲无码":25}
	pages = range(1,21) #采集的页数
