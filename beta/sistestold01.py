...
class ThreadGetTids(threading.Thread):
	...
	def run(self):
		...
		#logging.info(tids)			
		for tid in tids:
			print "page is:%s   tid is:%s	title is:%s"%(self.page,tid[0],tid[1])
			t[tid,page] = ThreadGetPics(tid[0],self.page,tid[1])  #然后把标题当参数传给getpic，这样一个循环搞定,这里应该三个参数t[tid,page]改成三个
			t[tid,page].start()
			t[tid,page].join()	

class ThreadGetPics(threading.Thread):
	sis001.forumurl = tid[0] = tid[1] = page = None
